...
function incrementUploadCountTest(metrics) {
  metrics.total_upload_count += 1;
}

function storeTestData(data, next) {
  if (
    !(
      data.fileName &&
      data.fileType &&
    ...
    next(new Error('All data is not present.'));
  }

  const dateOfExpiration = new Date(
    Date.now() + Number(data.expirationHours) * 3600 * 1000
  ).toUTCString();
    ...
    rows: [
      {
        ...
        download_count: data.maxDownloads,
        expiration: dateOfExpiration,
        id: 1,
        max_download_count: data.maxDownloads,
        name: data.fileName,
        type: data.fileType,
      },
    ],
...
app.post('/', async (req, res, next) => {
  ...
  try {
    const tuple = test ? storeTestData(req.query, next) : '';
    // await storeDataInPostgres(req.query, next);
    ...
    return res.status(200).send(JSON.stringify(tuple.rows[0]));
  } catch (err) {
